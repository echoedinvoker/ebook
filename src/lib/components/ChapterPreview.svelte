<script lang="ts">
	...
	let selectedExcerpt = $derived(
		chapters.find((chapter) => chapter.number === selectedChapterNumber)?.excerpt || ''
	);
	$inspect(selectedChapterNumber);
	$inspect(selectedExcerpt);
</script>

{#snippet chapterList(chapter: Chapter)}
	<li>
		<button
			class={`chapter-title ${chapter.number === selectedChapterNumber ? 'selected-chapter-title' : ''}`}
